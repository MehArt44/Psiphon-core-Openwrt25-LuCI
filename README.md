...
قبل از شروع اسکریپت‌ها، وضعیت فایل‌ها در روتر به شرح زیر خواهد بود (فایل کانفیگ در مرحله بعدی مستقیماً توسط ترمینال ایجاد می‌شود و نیازی به انتقال دستی آن نیست):
...
| ۲ | **فایل تنظیمات** | `psiphon.config` | `/usr/bin/psiphon.config` | توسط دستور مرحله ۳ مستقیماً روی روتر ساخته می‌شود |
...
⚠️ **نکته مهم:** این دستور را به صورت **کاملاً مستقل و جداگانه** در ترمینال روتر پیست کنید و اینتر بزنید تا فایل تنظیمات بدون تداخل با اسکریپت‌های بعدی، با موفقیت ایجاد و بسته شود:
...
> 🔍 **تست صحت ساخت فایل:** پس از اجرای دستور بالا، می‌توانید با زدن کامند `cat /usr/bin/psiphon.config` مطمئن شوید که محتویات فایل به درستی ذخیره شده است.

---

## ⚡ ۴. اسکریپت راه‌انداز داینامیک و هوشمند (با قابلیت انتخاب کشور)

این اسکریپت به صورت کاملاً خودکار فایل کانفیگی که در مرحله قبل ساختید را باز کرده، کشور انتخابی را در آن اعمال می‌کند، آی‌پای داخلی روتر شما را پیدا کرده و پورت‌های تصادفی سایفون را استخراج می‌نماید.
...
## 🗑️ ۹. حذف کامل و بی‌بازگشت سایفون (Uninstall)
