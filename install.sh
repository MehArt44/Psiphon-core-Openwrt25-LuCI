...
# مرحله 2: شناسایی معماری سی‌پی‌یو
ARCH=$(uname -m)
echo "[*] CPU Architecture detected: $ARCH"

# تطبیق معماری با نام فایل‌های موجود در Release v1.0.0 مخزن
case "$ARCH" in
    ...
    *) echo "[!] Unsupported architecture for auto-download: $ARCH"; exit 1 ;;
esac
...
echo "[*] Downloading Psiphon core ($DL_ARCH) from Release $VERSION..."
mkdir -p /usr/bin
wget -O /usr/bin/psiphon-core "$BIN_URL"
if [ $? -ne 0 ]; then
    echo "[!] Error downloading Psiphon core. Please check your internet connection or URL."
    exit 1
fi
chmod +x /usr/bin/psiphon-core
echo "[*] Psiphon core successfully installed."

# مرحله 1: دانلود پوشه مخزن و قرار دادن در مسیر خودش
echo "[*] Downloading and extracting repository files..."
wget -O /tmp/repo.zip "https://github.com/$REPO/archive/refs/tags/$VERSION.zip"
if [ $? -eq 0 ]; then
    unzip -o /tmp/repo.zip -d /tmp/
    # کپی کردن محتویات اکسترکت شده به مسیر روت (/) روتر
    # نام پوشه اکسترکت شده معمولاً به فرم نام_مخزن-نسخه است
    cp -r /tmp/Psiphon-core-Openwrt25-LuCI-1.0.0/* /
    rm -rf /tmp/repo.zip /tmp/Psiphon-core-Openwrt25-LuCI-1.0.0
else
    echo "[!] Could not download repo zip for tag $VERSION. Falling back to main branch..."
...
# مرحله 3: اجرای کدهای کانفیگ (GROUP 1 تا 7)
