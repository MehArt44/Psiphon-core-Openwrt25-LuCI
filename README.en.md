...
# 🛠️ 1. Guide to Compiling the Binary File (on PC) – PowerShell / Compiled for Multiple CPU Architectures

To build a custom executable file for your router, first make sure Go is installed on your system. Then open the terminal and run the following commands based on your router's CPU architecture:

Download the official source code of the Psiphon core from the GitHub repository:
...
Compile for 64-bit routers (Aarch64 / ARM64 like GL.iNet MT3000 / MT2500):
...
Compile for 32-bit routers (ARMv7 like Google Wifi AC-1304):
...
# 🚀 2. Transferring Files to the Router

After compilation is complete, transfer the output file `psiphon-core` and the `psiphon_data` folder using tools like MobaXterm or SCP to the following paths on the router:
...
# 📁 3. Deploying Infrastructure and Complete Graphic Panel Code

The code block below is an all-in-one script. Copy it completely and paste it into the router terminal. This script creates all LuCI structural files, UCI settings, dashboard JavaScript code (along with complete buttons and fields), and security permissions all at once:
	...
	load: function() {
		return Promise.all([
			...
			if (!document.getElementById('noto-emoji-font')) {
				...
				style.innerHTML = `
					/* Button styles with startup/stop colored borders */
					...
					/* Network structure and containers */
...
# 💡 Commands for Testing, Running, and Managing the Service
...
* **Enable automatic execution after router startup:**
```bash
...
# 🗑️ 8. Complete and Permanent Uninstallation of Psiphon from the System (Uninstall)

If for any reason you wish to remove all settings, binary files, databases, and LuCI panel menus of Psiphon without leaving any trace, run the integrated script below in the router terminal:
...
# Clean up Psiphon-related firewall rules
for sec in $(uci show firewall | grep -E "name='psiphon'|dest='psiphon'|src='psiphon'" | cut -d. -f1,2); do
    uci delete "$sec" 2>/dev/null
done
uci commit firewall

# Completely delete files, binaries, and directories
...
# Clear LuCI cache and restart system services
rm -rf /tmp/luci-indexcache* /tmp/luci-modulecache/ /var/luci-indexcache*
/etc/init.d/rpcd restart
/etc/init.d/uhttpd restart
/etc/init.d/firewall restart
